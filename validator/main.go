		...
		if cmd.Args().Len() < 1 {
			...
			if o != "" {
				...
				if ext != ".yaml" && ext != ".yml" && ext != ".json" {		
					log.Fatalf("Unknown extension on output file %v", o)
				}
			}				

			jsonInterface, yaml := validateConfiguration(configFilePath)
				
			if ext == ".json" {
				//j, _ := json.Marshal(jsonInterface)
				jsonToFile(jsonInterface, o)
			} else{
				jsonToFile(yaml, o)
			}
...
func validateConfiguration(configFile string) ([]byte, []byte) {
	...
	expandedConfig, b := decodeFile(configFile)
	...
	// d, _ := yaml.Marshal(expandedConfig)

	return expandedConfig, b
}

func decodeFile(file string) (interface{}, []byte) {
	...
	if ext == ".yaml" || ext == ".yml" {
		...
		if len(y.Content) > 0 {
			...
			if err := json.Unmarshal(j, &jsonInterface); err != nil {
				log.Fatalf("Invalid json file from yaml file %s: %+v", file, err)
			}
		}
...
func jsonToFile(jsonBytes []byte, outFile string) {
